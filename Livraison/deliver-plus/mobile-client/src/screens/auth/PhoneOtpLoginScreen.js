...
export default function PhoneOtpLoginScreen() {
  ...
  const handleSendOtp = async () => {
    ...
    const result = await sendOtp(phone.trim(), firstName.trim() || 'Client', lastName.trim() || undefined);
    if (result?.success) {
  ...
  useEffect(() => {
    if (otp.length === 6 && !expired && !loading) handleVerifyOtp();
  }, [otp]);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.logoWrap}>
            ...
            <Text style={styles.title}>Amder</Text>
            <Text style={styles.subtitle}>
              {step === 1
                ? 'Entrez votre numéro pour commander'
...
const styles = StyleSheet.create({
  ...
  logo:         { width: 72, height: 72, borderRadius: 20, backgroundColor: COLORS.purpleLight, alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
