...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    api.get('/orders?status=en_route&limit=1').then(r => {
      setActiveOrder((r.data.orders || [])[0] || null);
      setLoading(false);
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {!loading && activeOrder && (
          ...
          >
            ...
            <Text style={{ color:'#CECBF6', fontSize:22 }}>›</Text>
          </TouchableOpacity>
        )}
          ...
          {SERVICES.map(s => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('NewOrder', { serviceType: s.key })}
              >
        ...
        <View style={styles.card}>
          {[
            ...
            { step:'3', label:'Suivi en temps réel',    desc:'Suivez votre livreur en direct' },
          ].map(item => (
            <View key={item.step} style={styles.howRow}>
              <View style={styles.stepNum}>
                <Text style={{ color: COLORS.purple, fontWeight:'700', fontSize:13 }}>{item.step}</Text>
              </View>
              <View style={{ flex:1 }}>
