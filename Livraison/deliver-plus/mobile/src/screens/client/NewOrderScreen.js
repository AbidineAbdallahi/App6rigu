...
// Champ de recherche d'adresse avec suggestions Nominatim
function AddressField({ label, icon, value, onSelect, placeholder }) {
  ...
  const onChangeText = (t) => {
    setText(t);
    onSelect(null); // invalide la sélection précédente
...
export default function NewOrderScreen({ route, navigation }) {
  ...
  // Position GPS actuelle → point de départ par défaut
  useEffect(() => {
    (async () => {
      ...
      >
        ...
        {pickup && delivery && (
          <View style={st.summary}>
            ...
            <View style={st.summaryRow}>
              <Text style={st.summaryLabel}>Rayon de diffusion</Text>
              <Text style={[st.summaryVal, { color: COLORS.muted }]}>{BROADCAST_KM} km</Text>
            </View>
          </View>
        )}
